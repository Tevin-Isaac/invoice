...
// HASH160(publicKey))
function recoverPubkeyHashP2PKH(bitcoinBase58Address: string): string {
  ...
  if (version === 0 || version === 111) return hexlify(hash)
...
// HASH160(publicKey))
function recoverPubKeyHashP2WPKH(bitcoinBech32Address: string): string {
...
function toNestedSegwitAddress(
  ...
  const scriptPubKey = ripemd160(
    sha256(solidityPacked(["bytes2", "bytes20"], ["0x0014", pubKeyHash])),
  )
