...
function checkIfAddressStartsWith(bitcoinAddress: string, prefixes: string[]) {
  return prefixes.some((prefix) =>
    bitcoinAddress.toLowerCase().startsWith(prefix),
  )
}

// HASH160(publicKey))
function recoverPubkeyHashP2PKH(bitcoinBase58Address: string): Hex {
  ...
  if (version === 0 || version === 111) return toHex(hash)

  throw new UnsupportedBitcoinAddressError()
}

// HASH160(0014 | HASH160(publicKey))
function recoverRedeemScriptHashNestedSegwit(
  bitcoinBase58Address: string,
): Hex {
  const { hash, version } = address.fromBase58Check(bitcoinBase58Address)
...
// HASH160(publicKey))
function recoverPubKeyHashP2WPKH(bitcoinBech32Address: string): Hex {
  ...
  if (data.length !== 20) throw new UnsupportedBitcoinAddressError()
...
function toNestedSegwitAddress(
  compressedPublicKey: string | ByteArray,
  isTestnet = false,
): string {
  if (compressedPublicKey.length !== 66) {
    ...
    throw new Error(
      `The value [${compressedPublicKey}] is not a compressed public key`,
    )
  }

  const pubKeyHash = ripemd160(sha256(`0x${compressedPublicKey}`))
  const scriptPubKey = ripemd160(
    sha256(encodePacked(["bytes2", "bytes20"], ["0x0014", pubKeyHash])),
  )
  ...
  const checksum = toBytes(sha256(sha256(versionedData))).slice(0, 4)

  // The parameters passed to the `concat` function must be of the same type.
  // Therefore, we need to convert `versionedData` into bytes form.
  return bs58.encode(concat([toBytes(versionedData), checksum]))
}
 ...
 *          otherwise false.
 */
function isNestedSegwitAddress(bitcoinAddress: string, publicKey?: string) {
  const hasCorrectPrefix = checkIfAddressStartsWith(
    bitcoinAddress,
    supportedAddresses.P2SH_P2WPKH.prefixes,
  )
...
function recoverTruncatedBitcoinAddress(
  ...
  const isP2PKH = checkIfAddressStartsWith(
    bitcoinAddress,
    supportedAddresses.P2PKH.prefixes,
  )
  ...
  const isP2WPKH = checkIfAddressStartsWith(
    bitcoinAddress,
    supportedAddresses.P2WPKH.prefixes,
  )
...
function isP2WPKHAddress(bitcoinAddress: string) {
  try {
...
function isP2PKHAddress(bitcoinAddress: string) {
  try {
...
function isMainnetAddress(bitcoinAddress: string) {
  return checkIfAddressStartsWith(
    bitcoinAddress,
    networkAddressPrefixes.mainnet,
  )
}

function isTestnetAddress(bitcoinAddress: string) {
  return checkIfAddressStartsWith(
    bitcoinAddress,
    networkAddressPrefixes.testnet,
  )
}
