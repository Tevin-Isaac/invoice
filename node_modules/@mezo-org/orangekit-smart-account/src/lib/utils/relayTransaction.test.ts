...
describe("relayTransaction", () => {
  ...
  let request: BaseRelayParams

  beforeAll(() => {
    relay = new GelatoRelay()
    relayApiKey = "fakeApiKey"
    request = { chainId: 1n, data: "0x00", target: "0x" } as BaseRelayParams
  ...
  describe.each([
    ...
    "when the sponsored call was $taskDescription",
    ({ taskState, lastCheckMessage }) => {
      beforeAll(async () => {
        jest
          .spyOn(relay, "getTaskStatus")
          // @ts-expect-error We want to mock only used fields in our code.
      ...
      it("should throw an error", async () => {
        await expect(
          relayTransaction(relay, relayApiKey, request, backoffOptions),
        ).rejects.toThrow(
          `Relayed transaction failed with hash ${transactionHash}; ${lastCheckMessage}`,
        )
      })
    },
  )
  ...
  describe("when the task with a given ID is not yet available", () => {
    const error = new Error(
      "GelatoRelaySDK/getTaskStatus: Failed with error: Status not found",
    )

    let result: Promise<{ transactionHash: string | undefined }>
