import { isAddressable, resolveProperties, TransactionRequest } from "ethers"
import {
  TransactionSender,
...
export default class MezoTransactionSender implements TransactionSender {
  ...
  constructor(relayEndpoint?: string, refundReceiver?: string) {
    this.relayEndpoint =
  ...
  async sendTransaction(
    ...
    try {
      const response = await fetch(this.relayEndpoint, {
        ...
        body: JSON.stringify({
          txTo:
            resolvedTx.to && isAddressable(resolvedTx.to)
