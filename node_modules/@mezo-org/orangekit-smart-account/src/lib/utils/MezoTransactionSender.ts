...
import { mezoMainnet, mezoTestnet } from "./chains"
...
export default class MezoTransactionSender implements TransactionSender {
  ...
  constructor(
    ...
    switch (chainId) {
      case mezoMainnet.id:
        this.relayEndpoint =
          relayEndpoint ?? "https://mezo.org/api/v2/relay/transactions"
        this.refundReceiver =
          refundReceiver ?? "0xf35c1a20303ddcA464Ebf5b3b023588Cf3181c8a" // Mezo Passport Mainnet Relayer
        break
      case mezoTestnet.id:
        this.relayEndpoint =
          relayEndpoint ?? "https://test.mezo.org/api/v2/relay/transactions"
        this.refundReceiver =
          refundReceiver ?? "0x6e80164ea60673D64d5d6228beb684a1274Bb017" // testertesting.eth
        break
      default:
  ...
  async sendTransaction(
    transactionRequest: TransactionSenderRequest,
  ): Promise<TransactionSenderResponse> {
    try {
      const response = await fetch(this.relayEndpoint, {
        method: "POST",
        body: JSON.stringify({
          txTo: transactionRequest.to,
          txData: transactionRequest.data,
        }),
        headers: { "Content-Type": "application/json" },
      })

      const { transactionHash } = (await response.json()) as {
        transactionHash: Hash
      }

      // eslint-disable-next-line no-console
      console.log(
        `Transaction successfully submitted with hash: ${transactionHash}`,
      )
