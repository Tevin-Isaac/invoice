...
contract BitcoinSafeOwner is ERC1271, LegacyERC1271 {
    ...
    /// Reverts when provided signature has s value in the upper range of
    /// secp256k1 curve.
    error InvalidSignatureS();

    /// Reverts when provided signature has v value that is unsupported by the
    /// contract.
    error InvalidSignatureV();

    /// Reverts when provided public key is not a valid secp256k1 curve point.
    ...
    function validateSignature(
        ...
        if (signature.length != 129) revert InvalidSignatureLength();
        ...
        if (!isOnCurve(uint256(x), uint256(y))) revert PubkeyNotOnCurve();
    ...
    /// @notice Works like ecrecover but validates s for malleability
    ///         concerns. Only signatures with s value in the lower range of
    ///         secp256k1 curve are considered valid.
    /// @dev If your wallet generates malleable signatures with s in the upper
    ///      range, calculate a new value of s by subtracting it from
    ///      0xFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFEBAAEDCE6AF48A03BBFD25E8CD0364141
    ///      and flip the value of v.
    function recover(
        bytes32 signedMessage,
        uint8 v,
        bytes32 r,
    ...
    /// @notice The order of the finite field of the secp256k1 curve.
    uint256 internal constant SECP256K1_P =
        0xfffffffffffffffffffffffffffffffffffffffffffffffffffffffefffffc2f;

    /// @notice Check if a public key (x, y coordinates) is a secp256k1 point.
    /// @dev A secp256k1 curve point has the equation y^2 = x^3 + 7 % P
    /// @param x The x-coordinate of the public key.
    /// @param y The y-coordinate of the public key.
    /// @return A boolean indicating whether the coordinates are on the curve.
    function isOnCurve(uint256 x, uint256 y) internal pure returns (bool) {
        uint256 x3 = mulmod(x, mulmod(x, x, SECP256K1_P), SECP256K1_P);
        uint256 y2 = mulmod(y, y, SECP256K1_P);
        return (addmod(x3, 7, SECP256K1_P) == y2);
    }
