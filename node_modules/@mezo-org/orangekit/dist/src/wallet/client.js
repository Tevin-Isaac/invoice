...
export function createOrangeKitClient({ address, bitcoinWalletProvider, rpcUrl, chainId, orangeKitSmartAccount, }) {
    ...
    return createWalletClient({
        ...
        async sendTransaction({ to, value, data }) {
            return (await orangeKitSmartAccount.sendTransaction(to ?? "0x0", (value ?? 0n).toString(), data ?? "0x0", await bitcoinWalletProvider.getAddress(), await bitcoinWalletProvider.getPublicKey(), (message) => bitcoinWalletProvider.signMessage(message))); // cast to Hex to match return type of sendTransaction
        },
    }));
}
