import { BitcoinAddressHelper, GelatoTransactionSender, MezoTransactionSender, OrangeKitSmartAccount, } from "@mezo-org/orangekit-smart-account";
...
function selectTransactionSender(chainId, gelatoApiKey) {
    switch (chainId) {
...
export function createOrangeKitConnector(id, name, icon, rpcUrl, initialChainId, bitcoinWalletProvider, relayApiKey) {
    ...
    return createConnector(({ chains, emitter }) => ({
        ...
        async setup() {
            ...
            orangeKitSmartAccount = await OrangeKitSmartAccount.init(initialChainId, rpcUrl, selectTransactionSender(initialChainId, relayApiKey));
        },
        async connect({ chainId }) {
            try {
                ...
                const doesBitcoinAddressNetworkMatchWithProviderChain = providerChainId === 1 || providerChainId === matsnetTestnetChain.id
                ...
                ethersSigner = OrangeKitEthersSigner.init(null, account, bitcoinWalletProvider, orangeKitSmartAccount);
                return {
                ...
                if (err instanceof Error) {
                    ...
                    throw new Error("Something went wrong when trying to connect the wallet.");
                }
            }
        },
        disconnect() {
        ...
        async getProvider(parameters) {
            ...
            if (!orangeKitSmartAccount ||
                ...
                orangeKitSmartAccount = await OrangeKitSmartAccount.init(selectedChainId, rpcUrl, selectTransactionSender(selectedChainId, relayApiKey));
